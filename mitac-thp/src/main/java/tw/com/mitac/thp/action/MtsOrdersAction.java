...
public class MtsOrdersAction extends DetailAction<MtsOrders> {
	@Override
	...
	protected QueryGroup getQueryRestrict() {
		// 以轉單後供應商才可見
		if (escapeJoin()) {
			QueryGroup newQueryGroup = new QueryGroup(new QueryRule("ordersType", "MA"));
			return newQueryGroup;
		} else {
			String as1 = Util.buildJoinTableFrontKey(getPersistentClass());
			QueryGroup newQueryGroup = new QueryGroup(new QueryRule(as1 + "_" + "ordersType", "MA"));
			return newQueryGroup;
		}
	}
	...
	public String turnToWo() {
		try {
			...
			if (BillStatusUtil.MTS_TURN_ORDER.equals(billStatus)) {

				List<String> qList = (List<String>) cloudDao.findProperty(sf(), MtsOrders.class, new QueryGroup(
						new QueryRule("sysid", bean.getSysid())), new QueryOrder[0], false, "vendorSysid");
				if (qList.size() != 1) {
					addActionError("查詢錯誤，無法轉單");
					return EDIT_ERROR;
				}
				if (qList.get(0).equals(bean.getVendorSysid())) {
					addActionError("請更改供應商後再行轉單");
					return EDIT_ERROR;
				}

				PropertyUtils.setProperty(bean, Util.BILL_STATUS, BillStatusUtil.MTS_WAIT_ORDER);
				bean.setIsOrdersToVendor(true);
				bean.setRemark(bean.getOrdersNote() + "\n" + bean.getRemark());
				bean.setOrdersNote("");
			} else {
	...
	// BillStatusUtil.MTS_WAIT_CONFIRM);
