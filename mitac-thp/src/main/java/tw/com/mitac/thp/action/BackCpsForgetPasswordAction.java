 ...
 * 後台忘記密碼
 */
public class BackCpsForgetPasswordAction extends BasisTenancyAction {
	final Base64 base64 = new Base64();

	protected CpsMember bean;

	public final CpsMember getBean() {
		return bean;
	}

	public final void setBean(CpsMember bean) {
		this.bean = bean;
	}
	...
	public String ajaxDoForgetPassword() {
		...
		String uuid = request.getParameter("uuid").trim(); // 抓畫面上輸入的email
		logger.debug("查詢忘記密碼畫面輸入的email:" + uuid);
		List<CpsMember> memberList = cloudDao.queryTable(sf(), CpsMember.class,
				new QueryGroup(new QueryRule("uuid", uuid)), new QueryOrder[0], null, null);
		if (memberList.size() == 0) {
			...
			for (int i = 0; i < memberList.size(); i++) {
				...
				String daoMsg = cloudDao.save(sf(), new UpdateStatement(CpsMember.class.getSimpleName(),
				...
				if ("success".equals(daoMsg)) {
					...
					new MailThread(new MailBean(memberList.get(0).getEmail(), cpsEmailTemplate.getEmailTitle(), contentStringList),
							getSendMailSetting()).start();

					resultMap.put("msg", getText("web.chcekMail"));// 我們已將查詢資料寄送到您填寫的信箱!
				} else {
	...
	public String confirmAging() {
		...
		if (StringUtils.isNotBlank(urlPath)) {
			...
			logger.debug("檢查時間+PK是否符合：" + memberList.size());
			if (memberList.size() == 0) {
	...
	// 修改密碼畫面
	public String backAjaxDoChangePassword() {
		resultMap = new HashMap();
		// resultMap.put("isSuccess", false);
		resultMap.put("msg", "尚未執行");

		logger.debug("後台處理忘記密碼部分");
		...
		// 查詢是否有這筆會員資料
		List<CpsMember> memberList = cloudDao.queryTable(sf(), CpsMember.class,
				new QueryGroup(new QueryRule(PK, urlSysid), new QueryRule("emailVerifyCode", "1427")),
				new QueryOrder[0], null, null);
		
		resultString = "";
		logger.debug("ChangePassword-Sysidcount:" + memberList.size());
		...
		if (memberList.size() > 0) {
			...
			if (StringUtils.isNotBlank(urlSysid)) {
				resultString = cloudDao.save(sf(), new UpdateStatement(CpsMember.class.getSimpleName(),
